...
| **Email deliverability** | Resend, domain verified, sends live. **"Warming"; no DMARC/bounce/complaint loop. Founder legal identity is resolved in local server source; deployment and received digest test remain unverified.** | 55 | Deliverability collapse at volume | High | DNS (DMARC), deployment/test | 1–2 d | P1 |
...
| 5 | **Owner actions**: after separate deployment authority, verify the Founder-resolved LLC footer by received digest test; run encrypt backfills; enable Stripe customer receipts | Med | ~0 | Med | Verifies digest + receipts | Small | Comms | <1 d | owner |
