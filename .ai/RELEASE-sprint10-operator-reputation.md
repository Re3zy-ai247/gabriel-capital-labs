...
- **Facts on the Event Fabric:** reuses `ARENA_POINTS_CHANGED@1` (XP grants) + `ACHIEVEMENT_UNLOCKED@1` (badges); adds only `OPERATOR_RANK_CHANGED@1` + `MILESTONE_REACHED@1` (refs-only, source `reputation`). Recorded via `appendEvent` under a trusted `systemIdentity`, idempotent, **no fanout**. Reputation emits facts; owns no downstream behavior.
