...
| P0-6 | **Owner verification/actions** — `COMPANY_POSTAL_ADDRESS — RESOLVED BY FOUNDER`; after separately authorized deployment receive/inspect the admin digest test, run encrypt backfills, enable Stripe customer receipts | Founder | <1 d | deployment authority | Med | Verifies digest + receipts | Comms/receipts | **Yes** |
...
- **Founder (today, the real critical path):** engage counsel (P0-1); buy Vercel Pro (P0-4); separately
  authorize the identity-only deployment/test when ready + run encrypt backfills + enable Stripe receipts (P0-6); tell me the **DB
