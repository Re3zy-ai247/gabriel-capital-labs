# CreditVector™ — Codex entry point
...
## gstack
Installed at `~/.Codex/skills/gstack` (drives the five-review gate). Use `/browse` from gstack for web browsing; never `mcp__claude-in-chrome__*` tools. Skills list: run `/help` or see gstack's README.
