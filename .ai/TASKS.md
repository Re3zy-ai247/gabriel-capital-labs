...
- [ ] After separate deployment authority, send and receive the admin Brief test digest; verify the exact LLC postal footer and unsubscribe headers. `COMPANY_POSTAL_ADDRESS — RESOLVED BY FOUNDER`; source is local/uncommitted, so delivery is not yet verified.
...
Verified already done (updated 2026-08-01): `List-Unsubscribe` + One-Click headers live in `lib/briefDigest.ts` · `*.tsbuildinfo` untracked & gitignored · `.env.example` documents `CRON_SECRET`; the former `COMPANY_POSTAL_ADDRESS` placeholder is superseded by canonical server-scoped `lib/companyIdentity.server.ts` (`SETUP_SECRET`/`STRIPE_PRICE_ID` remain live code paths, so keep their warnings).
