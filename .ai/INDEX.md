...
| `OPERATOR-IDENTITY.md` | Any identity/session/role/agency-membership/handle/profile work. Consolidating **index** of the canonical rules (principal = `currentAccount().id`; agency = mode not role; `managedByAgencyId` edge) + the PROPOSED Operator Identity Service package (the next implementation target). Cites `lib/session.ts`/`auth.ts`/`network/authz.ts`; does not redefine them. |
