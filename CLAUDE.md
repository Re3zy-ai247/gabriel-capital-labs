...
**✅ Brief YouTube embed SHIPPED (`11cc580`, 2026-06-24):** optional `videoUrl` on `BriefArticle` (self-heal column `ALTER TABLE … ADD COLUMN IF NOT EXISTS "videoUrl"` in `ensureBriefTables`). Host-allowlisted parser `youtubeVideoId`/`normalizeYouTubeUrl`/`youtubeEmbedUrl` in `lib/briefShared.ts` (youtube.com/youtu.be/youtube-nocookie.com only; rejects lookalike hosts, `@`-userinfo spoofs, `javascript:`/`data:`); stored canonical, **re-parsed at render** so a value can only yield a YouTube iframe. Admin field in `app/admin/brief/page.tsx`; validate/normalize/clear in both admin API routes (blank clears, bad link → 400); public article page renders the **no-cookie** player (`youtube-nocookie.com/embed?rel=0`, responsive 16:9, lazy) in place of the cover (OG image stays the branded cover). Compliance gate = GO-WITH-CHANGES, both landed: publish `confirm()` now attests the video makes no guaranteed-deletion/score claims, and a "hosted on YouTube by its original publisher" caption labels the embed. Guard: `scripts/youtube.test.ts` 25/25 (host-spoof + scheme-injection). **✅ Owner-verified on-device 2026-06-24:** the official CFPB "dispute an error" video (`youtube.com/watch?v=MzteNX5w1vA`, channel `cfpbvideo`) is embedded on the published CFPB article — no-cookie player replaces the cover, third-party caption + disclaimer render, summary stays attributed. `videoUrl` self-heal column confirmed working on prod.

**Top priorities:**
1. ~~YouTube embed field for Brief~~ — **DONE**.
2. **⏸️ Brief Phase 2b — COMMENTS: BUILT, COMMITTED LOCALLY, NOT YET PUSHED (2026-06-24).** Stopped at the 5-hr usage limit right before pushing. **RESUME = just `git push origin main`**, then wait ~2 min for Vercel, then probe + owner test (no rebuild needed; it's committed on `main`, tsc + build already green, compliance GO). The comments commit is the HEAD of `main` (after `b5e3f05`). **Probes after push:** `/brief/<slug>` 200 (comment thread renders), `POST /api/brief/comments` anon → 401, `/admin/brief/comments` 200 for admin. **Owner test:** sign in → comment on a published article (try a banned phrase like "guaranteed deletion" → expect a rephrase error), report a comment (admin gets email+push), moderate it at `/admin/brief/comments`.
   - **What shipped in the commit:** post-moderation comments, open to any signed-in account. `BriefComment` + `BriefCommentReport` self-heal tables (`lib/brief.ts`). `screenCommentBody()` REJECTS (422, never stores/rewords) prohibited-claim comments via `applyCompliance` flags; clean text stored verbatim. `briefCommentAuthorName()` = username/first-name only (privacy). Routes: `app/api/brief/comments/{route,[id]/route,[id]/report/route}`, admin `app/api/admin/brief/comments/{route,[id]/route}`. UI: `components/brief/BriefComments.tsx` (plain-text/React-escaped, char-capped, rate-limited 15/hr) on the article page; admin `app/admin/brief/comments/page.tsx`; "Moderate comments" link + flagged badge (`/api/admin/context` now returns `flaggedComments`, AdminTabs Brief badge = drafts + flagged). UGC disclaimer `BRIEF_COMMENT_DISCLAIMER`. Compliance GO-WITH-CHANGES, both landed (disclaimer "unverified… results aren't typical"; privacy policy discloses public comments). Guard: `scripts/brief-comment.test.ts` 18/18.

**✅ Brief Phase 2a SHIPPED (`9fdff06`, 2026-06-24): likes + bookmarks + social shares.** Self-heal `BriefReaction` table (article,user,kind) + denormalized `BriefArticle.likeCount`. `POST/DELETE /api/brief/react` (idempotent via unique index, rate-limited 200/hr, user from session only → no IDOR, atomic like-count). Likes = any signed-in account, public aggregate count on article bar + feed cards (no usernames = no UGC). Bookmarks = private, new `/brief/saved` (noindex) + "Saved articles" feed link. Shares = open to all, native Web Share / X / Facebook / LinkedIn / copy-link, text uses only the already-scrubbed `socialCaption`→title. `components/brief/EngagementBar.tsx` (optimistic + revert). Compliance GO; the one Low (privacy policy didn't list activity data) remediated in-shipment (`app/legal/privacy`). Guard: `scripts/brief-react.test.ts` 9/9.

**Brief roadmap (phased):**
4. **Phase 2 remaining:** comments (= priority #2 above). Also offered-but-unbuilt: stat/data cards (visuals #2), admin image-upload (#3, public-domain/.gov or licensed images only).
