...
**→ Accept-as-amended (adversarial verdict): Tier B = half Tier A, ≤12px, ≤1.009, normative.** The paragraph above's `≤4 px` / `≤1.008` were RC1's descriptive record of what RC1 happened to ship, not a ceiling RC2 is bound by; see §4.3, §4.4, and §11.4. RC2's per-signature Tier B halves reach `±12 px` translation (client-operations, evidence-archive, growth-threshold) and `1.009` scale (Kai Suite) deliberately — half of RC2's own raised Tier A amplitude, the design rule this section's opening paragraph now needs read against, not a regression past an RC1 limit. Passage duration (460 ms) is unchanged and still correct.
...
Classification is structural: every motion-bearing surface carries `data-cxos-motion-channel="<class>:<name>"` (space-separated when a single owner can run more than one). The one owner that does is `.districtEnvironment` itself (`stage.tsx` ~1814–1816), which carries all three: `continuous:chamber-breath transient:chamber-acquire scroll:depth-parallax`. The breath animates `.districtEnvironment` directly; the transient chamber-acquire and scroll depth-parallax animations both run on its `[data-plane="depth"]` child instead, which carries no channel attribute of its own and resolves its owner via `closest("[data-cxos-motion-channel]")` — i.e. `.districtEnvironment`. The evidence harness resolves a single applicable token per *running* animation from that animation's own computed timeline type and iteration count when its owner declares more than one token — never from the keyframe name. A legacy keyframe-name regex is retained only as a narrow backstop for the `unclassified-environment-animation` fail path (any running animation on a decorative/environment surface with no resolvable token), never to decide a class.
...
**CSS mechanism — mutual exclusion, not precedence.** Every RC2 continuous opt-in (`.ambientSweep`'s facility-sweep, both per-chamber `.districtEnvironment` breath rules, the client-operations blocked-lane pulse) restates the same thirteen-way quiet-state negation directly on its own selector — verbatim, each on its own selector, not a shared or inherited one (RC2 WP-FIX2 closed the gap: the two breath rules and the pulse previously lacked this negation entirely). This is checked against the QUIET kill list a few rules below it (gated on idle/attention/Kai/environment-motion/tier, ~css:8277 at RC2 WP-FIX2 HEAD) — **not** the unconditional `.room[data-cxos-environment] :is(...)` list nearer the top of the Living-mode block (~css:7808), a separate mechanism that retires every legacy decorative surface regardless of state. The facility sweep and the budget-1 breath rule are an exact specificity tie with that QUIET kill list, `(0,5,0)` on both sides — the tie that originally motivated the negation (RC1 left it unbroken, resolved only by source order, for the scroll opt-in specifically). RC2 WP-FIX2 also extended the kill list's own `:is()` to additionally stop `.districtEnvironment` itself and `.flowTrack b` (previously only `.ambientSweep` and `.districtEnvironment`'s descendants), which makes the kill list itself the more specific side by one element-level component; the budget-2 breath rule and the blocked-lane pulse were never exact ties with it in the first place (each carries at least one attribute/`:not()` component beyond the facility sweep's). None of that arithmetic is load-bearing: the negation makes every opt-in and the kill list structurally mutually exclusive — at most one can ever match a given element — so which side specificity favors no longer decides anything:
...
**Runtime safety net.** `useCxosRoomRuntime.ts` adds a `useLayoutEffect` that, the instant `livingEnvironment.motion` stops being `"active"`, walks `root.getAnimations({ subtree: true })` and considers any animation whose owning `[data-cxos-motion-channel]` ancestor's raw attribute string contains a `continuous:` token. **RC2 WP-FIX2 (F5) scoped what happens next per ANIMATION, not per owner.** An owner can declare more than one token on one attribute — `.districtEnvironment` is the sole example, carrying all three classes for three physically distinct animations (§11.1) — and the original net cancelled every animation sharing that owner whenever any of its tokens was `continuous:`, which force-cancelled a still-in-flight `transient:chamber-acquire` entry or the `scroll:depth-parallax` ViewTimeline animation too, an undisclosed over-reach the adversarial review caught. The fix reads `getComputedStyle` on THIS animation's own target/pseudo-element (never the shared owner) and cancels only when this animation's own `animationName` is absent from that computed list — i.e. the cascade has already disowned this specific animation, not merely some sibling on the same owner. This runs before the next paint (`useLayoutEffect`, not `useEffect`). It is belt-and-suspenders on top of the CSS gate, not a second policy or a replacement — it reads the identical `data-cxos-motion-channel` token grammar the CSS and the harness already treat as the single source of truth, and it is room-agnostic (no Agency-specific class names).

**Why the net exists — disclosed, most-likely-explained, not resolved.** A real long multi-step harness session (concurrent per-chamber Axe audits plus `animations:"disabled"` screenshot capture, under heavier main-thread load than any single constructed repro) showed one specific continuous animation — `.ambientSweep`'s facility-sweep, an 18-second linear-infinite loop — occasionally still reported as running well after `continuousAnimationBudget` read `0` in the DOM. The CSS negation is specificity-correct, and every constructed single-mechanism repro (atomic attribute flips against the real built stylesheet, repeated engage/settle cycling, a full seven-chamber navigation loop with real per-chamber Axe audits, scroll-linked measurement, arrival replay, and combinations of all of the above) canceled it correctly every time. The exact browser-level mechanism was not pinned down at RC2 WP-FIX time. **RC2 WP-FIX2 re-labels it: most likely a Playwright `animations:"disabled"` screenshot-capture cancel-and-replay instrumentation artifact, not a production defect** — that capture mode works by finishing/canceling and replaying running animations around the screenshot, machinery that exists only under the harness and never runs in a real browser session — but this remains unconfirmed, disclosed rather than claimed solved. A decisive computed-style-vs-`getAnimations` measurement is now captured directly in the evidence: every animation-ledger entry records a computed-style `animationName` (beside the WAAPI `name`) alongside the running entry, and the runtime net's own per-animation-name gate above reads the identical comparison live — so a future occurrence is diagnosable from the ledger instead of requiring a fresh repro session. This is distinct from, and should not be confused with, the `scroll:depth-parallax` specificity-tie bug described above, which *was* fully diagnosed and fixed structurally in the same investigation.
...
| Team recognition (+ stagger) | Team Operations · `.teamOrbit li > span` | `agencyTeamRecognition` | 860 ms (+90 ms / +180 ms staggers) | team-model activation, or chamber acquisition while the Team Specimen model is already active — conditional, not unconditional: the default Solo room renders `.teamSoloCore` and mounts no `.teamOrbit` to animate at all (RC2 WP-FIX2 correction, §11.10) | `transient:team-recognition` |
...
The `.kaiResponse` element (not the similarly named `.preparedArtifact`, which is orphaned CSS with no JSX consumer) is the live "intelligent response" beat: each submitted command mounts a new `.kaiResponse` article keyed by a fresh turn id. **RC2 WP-FIX2 correction (§11.10):** `.kaiResponse` lives inside the Kai Suite district shell, which — like every other chamber — carries the native `[hidden]` attribute whenever it is not the current chamber. Leaving and returning to Kai Suite is therefore a remount-equivalent event for this element exactly like the six chamber recognitions below, and the reveal re-fires on every return to Kai Suite while a prepared turn exists, not only once on the turn's original mount.
...
| 15 | §11.3 table, Team recognition row (RC2-authored; corrected during RC2 WP-FIX2's adversarial-gate remediation, not an RC1 statement) | "Fires on: chamber acquisition" (stated unconditionally) | Fires on team-model activation, or chamber acquisition while Team Specimen is already the active model — the default Solo room renders `.teamSoloCore` and mounts no `.teamOrbit` to animate at all |
| 16 | §4.4, Decorative light-intensity prose ("attention and idle reduce environmental-plane opacity to `0.30` and `0.46` respectively") | `0.30` / `0.46` | Reading dim is `0.42` (§11.5, moved up from RC1's `0.30`); settle opacity is per-signature `0.42–0.54` (§11.4), not a flat `0.46` |
