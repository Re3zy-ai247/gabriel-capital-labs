  ...
  return (
    <div className="space-y-3">
      <Section icon={<CircleAlert className="h-3.5 w-3.5" aria-hidden="true" />} title="Needs attention">
        {awaiting.length === 0 ? (
          <p className="text-xs leading-relaxed text-slate-500">Every brief has a first response. The queue is clear.</p>
        ) : (
      ...
      <Section icon={<Activity className="h-3.5 w-3.5" aria-hidden="true" />} title="Recently active">
        {active.length === 0 ? (
          <p className="text-xs leading-relaxed text-slate-500">Briefs with responses land here as the network works.</p>
        ) : (
          <ul className="space-y-2.5">
            {active.map((t) => (
      ...
      <Section icon={<Sparkles className="h-3.5 w-3.5" aria-hidden="true" />} title="Kai activity">
        {kaiThreads.length === 0 ? (
          <p className="text-xs leading-relaxed text-slate-500">
            Bring Kai into a brief and the answer lands here — statutes and process, never promises.
          </p>
        ) : (
          <div className="text-xs leading-relaxed text-slate-400">
            <p>
              ...
              {kaiThreads.length === 1 ? "brief carries" : "briefs carry"} a Kai answer.
            </p>
            {latestKai && (
