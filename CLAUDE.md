...
2. **✅ Brief Phase 2b — COMMENTS: SHIPPED & live (`d77accb`, pushed 2026-06-25).** Prod probes green: `/brief` 200, `/brief/<slug>` 200 (thread renders), `GET /api/brief/comments` 200 (public — confirms `BriefComment`/`BriefCommentReport` self-heal tables created on first request), `POST /api/brief/comments` anon → 401, `GET /api/admin/brief/comments` anon → 403, `/legal/privacy` 200. **⚠️ Owner test pending:** sign in → comment on a published article (try a banned phrase like "guaranteed deletion" → expect a 422 rephrase error), report a comment (admin gets email+push), moderate it at `/admin/brief/comments`. **This completes Brief Phase 2** (likes · bookmarks · shares · comments all live).
