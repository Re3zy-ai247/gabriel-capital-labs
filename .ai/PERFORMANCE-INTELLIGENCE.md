...
- **Improvement-measurement integrity (this service MEASURES improvement; the economy REWARDS it — [`CREDITVECTOR-ECONOMY.md §3`](CREDITVECTOR-ECONOMY.md) + §11).** Any "improvement" signal this service emits for reputation to reward must be **un-gameable**: measured against the operator's **best sustained prior baseline** (never a most-recent trough, so degrade-from-high-then-recover earns nothing) **AND a capability-adjusted / peer-cohort-relative baseline** (so a suppressed FIRST window — onset sandbagging — cannot become the reward baseline); over a **rolling sustained window** (absorbing spikes + regression-to-the-mean, but **NOT** seasonality — use **period-aligned same-season / year-over-year** comparison); bound to event **occurrence/attestation time, not operator log time** (with batch-log velocity caps + void on anomalous boundary log-clustering); gated by **minimum data sufficiency + activity thresholds** (no small-denominator "+100%"); requiring a **composite / pre-declared** metric (no cherry-picking one lucky KPI); with **org-growth/team-size/client-volume normalization** and a **voided comparison when the SOP/KPI definition changed**. Emit confidence + provenance; flag anomalies for hold. This is the measurement half; the reward-fairness half (cohorts, floors, manual review) is the economy's.
