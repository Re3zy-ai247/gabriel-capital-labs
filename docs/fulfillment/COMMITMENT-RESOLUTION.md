...
| F4 Free fulfillment (consume-after-void) | **ELIMINATED** | `attempt` dimension in the ledger key; settle requires an ACTIVE hold for (subject, attempt); settle-after-release refuses; re-authorization = attempt+1; adjacent `prior_attempt_still_active` guards added (W1 §S2 + deviation 2) |
...
| F9-iii Retry deduped by claims | **ELIMINATED (contingent on Claim shipping attempt-scoped from day one)** | Claim key `${subjectId}:${toStage}:${attempt}` in BOTH domains (W2 + W1 deviation 3 — W1 independently extended the wallet claim key with `attempt`, closing a hole the spine missed) |
...
| Join | Ruling |
|---|---|
| W3's proposed `RecoveryVerdictType` grammar vs W2's matrix `kaiCopyClass` handles | **W2's matrix keys are canonical.** W3's catalog re-keys mechanically (rename-only; copy text unchanged) — recorded as a pending one-pass edit, no content conflict found |
