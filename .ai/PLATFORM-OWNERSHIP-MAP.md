...
| **Reputation / trust (Vector XP)** | **`lib/reputation/**` (durable ledger + progression)** ; scoring policy stays `lib/arena/policy.ts` (ARENA-CONTRIBUTION-POLICY) | **BUILT (Sprint 10, dormant) — canonical** | Owns XP storage/progression/milestone truth + emits the progression facts (`ARENA_POINTS_CHANGED`/`ACHIEVEMENT_UNLOCKED`/`OPERATOR_RANK_CHANGED`/`MILESTONE_REACHED`). The v1 arena reconcile-on-read fold (`lib/arena/project.ts` consumers) remains the ACTIVE own-XP read path until Arena re-points to reputation projections at activation (documented successor pattern, same as the agency mechanism). Identity owns none of it. |
