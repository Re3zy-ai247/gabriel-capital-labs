...
Status: **index of EXISTING truth + RATIFIED Identity semantics + PENDING implementation.** The [Identity Constitution v1.0 Ratification Edition](IDENTITY-CONSTITUTION.md) is authoritative as of 2026-07-26. Existing runtime, schema, migrations, and flags remain implementation evidence; none was changed or activated by ratification.
...
**Ratified Identity Constitution (2026-07-26):** [IDENTITY-CONSTITUTION.md](IDENTITY-CONSTITUTION.md) is **RATIFIED**. The prior [clause-level audit](IDENTITY-CONSTITUTION-AUDIT.md) is retained as historical review evidence; its contradictions were resolved in the Ratification Edition. Controlled implementation is authorized only through the [implementation plan](IDENTITY-CONSTITUTION-IMPLEMENTATION-PLAN.md); Gate D, migrations, flags, merge, deployment, and activation remain separately gated.
...
## 5. Identity, Organizations, and Membership (ratified semantics; foundation dormant; implementation pending)

**Sprint 9 status (2026-07-21, merged; repository source confirmed at `origin/main` `e28bd68` on 2026-07-25):** the **foundation** is implemented in `lib/identity/**`, migration-first (`20260721120000_operator_identity`, additive 0 DROP) and fully **dormant behind `OPERATOR_IDENTITY_ENABLED` (fail-closed off)**. Built: durable `OperatorIdentity` (1:1 to account, lifecycle state machine — terminal states block resurrection), durable `Organization` (owner by id, unique slug — not the spoofable `agencyName`), `OrganizationMembership` (`@@unique(org,operator)`), the org-scoped RBAC permission map (`rbac.ts`, a PEP-consumable mapping, NOT enforcement), the auth boundary (`principal.ts` — consumes `currentAccount()`, never authenticates), 6 refs-only Security/Audit events on the Event Fabric, and the reserved `operator_profile` media boundary (hard-off, not built). The existing owner/global-`ADMIN` authorization and direct-active membership commands are dormant compatibility defects, not constitutional authority. Public profiles, managed-client credential login, educator verification, certifications, marketplace linkage, and every HTTP surface remain separately proposed or counsel-gated. No route, activation, deployment, or production migration was performed by this repository assessment.
...
**The Ratification Edition governs meaning; implementation is pending.** The existing physical `lib/identity/**` package may host the three semantic subdomains below without creating parallel owners or requiring service extraction.

**Semantic owners:** Identity owns OperatorIdentity enrollment and lifecycle; Organizations owns Organization state, ownership, and transfer; Membership owns admission, role, state, and removal. Consumer-management retains `managedByAgencyId`, agency mode, managed-client workspace selection, and service termination. Authentication retains credentials/session/principal resolution; Authorization/PEP enforces policy; Event Fabric transports only.
...
**Evidence facts:** Identity, Organizations, and Membership own their versioned lifecycle evidence and publish domain facts through the Event Fabric. `AGENCY_CLIENT_ADDED` and `AGENCY_MODE_ENABLED` remain Consumer-management facts; `SESSION_IMPERSONATED` remains Authentication/Admin-audit meaning. Colocation in the transport registry does not transfer ownership.
...
5. **Cross-organization rules** — an Operator may belong to more than one Organization. The live `managedByAgencyId` edge is a separate managed-consumer relationship, not membership.
...
**Controlled implementation prerequisites:** follow [IDENTITY-CONSTITUTION-IMPLEMENTATION-PLAN.md](IDENTITY-CONSTITUTION-IMPLEMENTATION-PLAN.md); contain the live managed-client deletion hazard first; approve a versioned Gate D extension before any seventh migration; preserve migration-first ordering; keep all new reachability fail-closed; obtain CCO/counsel approval before any public or cross-user profile surface.

### Activation-readiness assessment (2026-07-25 — VERIFIED repository source; no runtime or schema change)

The former `managedByAgencyId → OrganizationMembership` bridge is **constitutionally invalid**. The ratified replacement is read-only reconciliation classification followed, when independently eligible and consented, by explicit Operator enrollment and Membership commands. No conversion or backfill is authorized.

| Concern | Repository truth / required future decision |
| --- | --- |
| Canonical truth today | `managedByAgencyId` is the LIVE `User`→`User` agency-managed-client edge: a passwordless consumer workspace is managed by an agency account. It remains the sole active agency-client authorization and entitlement relation until its own separately governed Consumer-management replacement. |
| Separate dormant domain fact | `OrganizationMembership` is an `OperatorIdentity`→`Organization` relation owned by Membership; an organization owner may own zero or many organizations, and a managed client need not have an `OperatorIdentity`. The legacy edge cannot prove membership, consent, role, or organization. |
...
| Reconciliation and history | Automatic backfill or membership creation is permanently forbidden. A future read-only classifier may preserve source evidence, explicit candidate selection/ambiguity, deterministic ordering, stable command identity, and non-cascading history; only an independently valid Membership may later be classified as reconciled. |
...
**Ratified resolution:** the Constitution now defines enrollment, consent, reconciliation classifications, lifecycle authority classes, suspended-state behavior, owner protection, and Gate D extension law. Implement these rules through the staged plan; no managed-consumer row may authorize or automatically create OperatorIdentity, Organization, or Membership state.
...
RATIFIED Identity semantics + index of VERIFIED current truth; implementation is pending. Amend constitutional meaning only through [IDENTITY-CONSTITUTION.md §24](IDENTITY-CONSTITUTION.md#24-amendment-and-supersession). Implementation must use small migration-first slices and obey the plan's explicit ADR triggers; ordinary implementation within the ratified owners does not create a new authority layer. Dormant flags remain fail-closed; nothing in this document enables a capability.
