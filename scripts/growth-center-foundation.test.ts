...
try {
  ...
  check("production identity hard-disables review despite both overrides", !reviewBuildAllowed() && growthCenterPreviewEnabled());

  process.env.NEXT_PUBLIC_VERCEL_ENV = "preview";
  delete process.env.NEXT_PUBLIC_CXOS_REVIEW;
  check("Vercel preview requires and accepts the subordinate flag", reviewBuildAllowed() && growthCenterPreviewEnabled());

  delete process.env.NEXT_PUBLIC_VERCEL_ENV;
  delete process.env.NEXT_PUBLIC_CXOS_REVIEW;
  check("unknown production identity fails closed", !reviewBuildAllowed());
...
const gateIndex = page.indexOf("if (!reviewBuildAllowed() || !growthCenterPreviewEnabled())");
