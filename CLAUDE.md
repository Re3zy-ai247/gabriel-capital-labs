...
2. **Brief Phase 2b — COMMENTS (next task).** Post-moderation model (agreed): comments appear immediately, run through `applyCompliance` on write, reportable → admin queue (reuse the `CommunityReport` pattern + admin email/push alerts). Open to **any signed-in account** (free incl.), like the rest of Phase 2. Needs: a `BriefComment` self-heal table (article,user,body), `POST/GET /api/brief/comments` (rate-limited, scrubbed, length-capped via a `*Shared` const), a client comment thread on the article page, a UGC disclaimer, report wiring, and an admin moderation surface (extend `/admin/reports` or add a Brief-comments view). Run `/compliance-review` (this is the real public-UGC gate) before ship.
