...
- **Resend transactional email LIVE** — domain `creditvector.app` **VERIFIED** in Resend (DNS added at **Squarespace** = the domain's DNS host; Google Workspace apex email untouched). `lib/email.ts` `sendEmail`/`sendAdminEmail`. Prod env: `RESEND_API_KEY`, `RESEND_FROM=CreditVector <no-reply@creditvector.app>`, `ADMIN_EMAIL=reygabriel.ai247@gmail.com`. Delivers to ANY recipient now.
...
- **Web Push (PWA phone notifications) SHIPPED — needs on-device retest** — `worker/index.js` (next-pwa custom worker: push+notificationclick), `lib/push.ts` (`web-push` + `PushSubscription` self-heal; `sendPushToUser`/`sendPushToAdmins`; fails safe), `POST /api/push/{subscribe,unsubscribe}`, `components/PushToggle.tsx` ("Enable phone alerts" in Settings). Wired to Brief-draft + community-report alerts. Prod env: `NEXT_PUBLIC_VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`. ⚠️ **Android enable was hanging; hardening (timeouts + clear errors) shipped `d9a554f`. NOT yet confirmed on a device.**
...
## ▶ NEXT SESSION — resume here (updated 2026-06-23)
**In-flight / top priorities:**
1. **Web Push on-device retest (Android).** Owner hit a hang on "Enable phone alerts"; the hardening (timeouts + clear errors, commit `d9a554f`) just deployed. → Owner: **hard-reload twice** (PWA caches JS), Settings → Phone Alerts → **Enable**, tap **Allow** on the permission prompt; create a Brief draft to test the push lands. If it fails, the toggle now shows the exact reason (SW timeout / blocked / sign-in / push-service) — report that. (iPhone needs **Add to Home Screen** first; Android Chrome works in-browser.)
2. **Publish the first Brief article.** The feed is EMPTY until the owner publishes. A CFPB draft (2025 Consumer Response Annual Report; ~5.1M complaints, ↑249%) was prepared in the admin form → `/admin/brief` → Save draft → **Publish** (confirm the numbers vs the linked source).
3. **YouTube embed field for Brief (offered, NOT built).** Add `videoUrl` to `BriefArticle` (self-heal `ALTER TABLE "BriefArticle" ADD COLUMN IF NOT EXISTS "videoUrl" TEXT`), an admin field validated to youtube.com/youtu.be, and an official `<iframe>` embed on the article page. Legal: embedding via the official YouTube player is fine; never download/reupload; prefer official channels.
