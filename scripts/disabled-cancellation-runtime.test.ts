...
// lib/session.ts, over in-file fakes for next-auth / prisma / next/headers /
// Stripe. Nothing here is a source-text assertion; the scope assertions that must
...
Mod._load = function patched(this: unknown, request: string, parent: unknown, isMain: boolean) {
  if (request === "next-auth") return { getServerSession: async () => (sessionUser ? { user: sessionUser } : null) };
...
async function run() {
  ...
  const state5 = await session.sessionAccountState();
  ...
  const state5b = await session.sessionAccountState();
  ...
  const c6 = await selfCancel.POST(
  ...
  const c6b = await selfCancel.POST();
  ...
  const c10 = await selfCancel.POST();
  ...
  const c10c = await body(await selfCancel.POST());
  ...
  retrieveThrows = true;
  signInAs(suspendedPayer);
  const c11 = await selfCancel.POST();
  ...
  const c11c = await selfCancel.POST();
  ...
  const c11d = await selfCancel.POST();
  ...
  const c8b = await selfCancel.POST();
  ...
  signInAs(noCustomer);
  const c9 = await selfCancel.POST();
  ...
  check("9 RUNTIME a row with a customer but NO subscription is refused (404)",
    (await selfCancel.POST()).status === 404);
  ...
  signInAs(activePayer);
  const c1b = await selfCancel.POST();
  ...
  const g1 = await body(await selfCancel.GET());
