...
- **Sustained-performance requirement:** a transient spike does not qualify; improvement must hold across a rolling window (which absorbs short-term **noise** + regression-to-the-mean). **Window length does NOT absorb SEASONALITY** (periodic structure persists across the whole window) — credit work is strongly seasonal, so improvement uses **period-aligned (same-season / year-over-year) comparison or explicit seasonal adjustment**, never a raw Q3→Q4 delta (see §11).
...
|---|---|---|---|---|---|---|
| **Deliberate-degradation (from an established HIGH)** | tank KPIs, then "recover" to mint Most-Improved | competition/XP recognition | **best-prior-baseline** + floor + sustained-window (§3) | anomaly hold; invalidation record | floor/window values | **LOW** (this variant covered) |
| **Onset baseline sandbagging** (the sibling best-prior does NOT reach) | suppress the FIRST measured window above the floor (no prior high exists to compare to), then reveal true ability → huge "improvement" | Most-Improved / Operational-Improvement | §11 capability-adjusted / peer-cohort-relative baseline | reclassify; hold | anti-sandbag baseline rule | **HIGH** (open until §11 rule set) |
...
| **Direct XP↔cash/credit conversion** | convert affiliate cash to XP or vice-versa | launder pay-to-win | **PGE-4 separation** — distinct ledgers + a **guard-pinned cross-instrument no-read invariant** (§11); doctrine alone is not enforcement | n/a (no direct path) | no-read guard | LOW |
| **INDIRECT pay-to-win laundering** | cash buys paid subscriptions / funds referred signups → mints Business-Growth XP (the path best-prior + caps only *bound*, not close) | buy reputation indirectly | self-funded / same-payer / same-beneficiary referred memberships are **ineligible** Business-Growth evidence (§11); dimension caps | claw back; audit | referral eligibility + cap values | **HIGH** (bounded, not closed) |
| **XP→value leak via Reward Claims** | milestone→entitlement→cash-valued claim (subscription month, voucher) turns reputation into money, dodging the promotional-credit gate | monetize XP | cash-valued reward claims inherit the **promotional-credit governance** (legal/accounting/tax/liability/expiration/no-cash-redemption) — §11 + ADR-0038 §4 | revoke; audit | reward-claim liability review | **HIGH** (until claims are gated) |
...
## 11. Adversarial hardening — binding prerequisites (2026-07-20 economy review)
A 5-agent economic adversarial pass (verdict ADD_CONTROLS) confirmed these gaps in the first draft. Each is a **binding prerequisite before any weight, competition, or public surface goes live** — not optional. None is built.

**Improvement measurement (with `PERFORMANCE-INTELLIGENCE §2.1`):**
- **Anti-onset-sandbag:** a suppressed FIRST measured window must not become the reward baseline. Use a **capability-adjusted or peer/cohort-relative** improvement baseline (best-prior only defends degrading an already-established high — §9).
- **Seasonality:** period-aligned (same-season / YoY) comparison or explicit seasonal adjustment — window length absorbs noise, not periodic structure.
- **Occurrence-time binding:** measurement binds to event **occurrence/attestation time, not operator log time**; batch-logging velocity into a window is capped; windows with anomalous boundary log-clustering are voided (closes self-logged window-boundary arbitrage).
- **Multiple-comparisons:** improvement must be on a **composite / pre-declared** metric set (or a multiple-comparisons correction) so one lucky KPI out of many cannot alone mint recognition.

**Competitions, cohorts & privacy (§5):**
- **Veteran-in-fresh-org:** cohort assignment uses an operator-experience / prior-org signal so a veteran's new org is not seeded into the rookie/new bracket.
- **Growth-triggered promotion:** an **event-driven immediate** promotion out of an easy bracket, independent of the slow demotion cadence (resolves the stable-vs-fresh tension); or freeze a competitor's cohort at **entry**.
- **Provisional prizes:** a competition prize (entitlement/claim) is **provisional until every constituent award clears its evidence-maturation/clawback window**, with a re-rank-and-reclaim path on reversal; competition cycle length **≥ the §3 improvement sustained-window**.
- **Rate-not-count categories:** every non-"Most-Improved" competition scores on a rate/percentage/normalized-within-cohort/per-capita metric, never raw count with a quality tiebreak.
- **§1679b BEFORE consent:** operator display-consent cures operator privacy, **not** the CROA §1679b implied-results harm. Every public competition/profile **category name + framing must be independently §1679b-clean** before any consent can unlock it — consent is necessary, never sufficient.
- **Client-side consent + de-identification:** any **client-derived** outcome (Verified Client Outcomes, Client Engagement Growth) feeding a cross-user competition or public profile requires a **consumer-side consent + de-identification / k-anonymity** gate, distinct from operator consent (third-party consumer privacy + FCRA/GLBA).

**Referral / affiliate / instrument separation (§6, ADR-0038 §4):**
- **Full-liability revocability:** ALL referral-derived awards (XP fold, **latched milestone**, entitlement, and any consumed reward claim) stay revocable for the entire card-network **chargeback-liability** period — this **overrides the §4B milestone-latch** for revocable-evidence-derived milestones (they are provisional until the evidence matures).
- **Ineligible referrals:** self-funded / same-payer / same-beneficiary / related-party referred memberships are ineligible Business-Growth evidence; "related party" gets an un-fakeable definition; star-topology + circular topologies detected.
- **Qualified referral** = payment + independent un-fakeable **activation** (not paid-but-dormant) + retention past full chargeback liability — defined before any referral weight mints.
- **Affiliate cash** has **no control today**; it needs its own pend/clawback/full-liability design as a named prerequisite **before** the §9 refund-window claim over it is valid.
- **No-read invariant:** PGE-4 separation is enforced by a **guard-pinned cross-instrument no-read invariant** in any future affiliate/promotional-credit ledger, not by doctrine alone.
- **Cash-valued reward claims** (subscription-month add-on, consultation/exam vouchers, bundles, event admission) inherit the **promotional-credit governance** (ADR-0038 §4) — "claiming never deducts XP" is not sufficient; these carry real monetary value.

**Insider / governance / marketplace / appeal:**
- **Maker-checker everywhere:** two-independent-approver control extends beyond XP-amount edits to **Entitlement grant/revoke, Reward-Claim issuance, milestone/threshold edits, and every compensating/invalidation ledger write** (magnitude caps are meaningless on binary/structural grants). A real independent second approver must be named given the `{USER, ADMIN}`-only role model.
- **Marketplace facilitator:** publish-time screening is not enough — add seller identity verification/KYC, **ongoing post-approval monitoring**, and a consumer-complaint/takedown mechanism; the consultation-voucher reward claim passes the same content screen; a written facilitator-liability + CSO/CROA-status position is required (ADR-0037 §5).
- **Appeal mechanism:** a fraud hold has an **SLA + auto-reversal of unadjudicated holds**, an interim-treatment rule during appeal, an independent reviewer, and a **managed-consumer (passwordless) appeal path** — so an erroneous fraud classification is never permanent punishment.
- **Erasure vs ledger:** a CCO-written retention position reconciles data-subject erasure with permanent audit-actor retention, defines post-erasure disposition of already-granted entitlements/claims, and forbids using erasure to destroy fraud/compensating evidence.
- **Identity primitives:** a named identity-verification / Sybil-resistance primitive, an educator-verification authority, and the un-fakeable class-E client-activation signal — all before any identity-gated economic weight or educator/marketplace surface activates.
