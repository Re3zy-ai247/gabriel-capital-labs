...
const identity = source("lib/companyIdentity.server.ts");
check("identity is held in a server-suffixed module", identity.includes("canonical server-scoped company identity") || identity.includes("server-rendered legal"));
check("identity does not read environment configuration", !identity.includes("process.env"));
check("identity does not define public environment configuration", !identity.includes("NEXT_PUBLIC_COMPANY_"));
...
check("digest fails closed if canonical postal identity is unavailable", digest.includes("!COMPANY_POSTAL_ADDRESS.trim() || !COMPANY_POSTAL_ADDRESS_INLINE.trim()"));
...
check("rendered digest preserves display-name branding", renderedDigest.text.includes("CreditVector by Gabriel Capital Labs"));
...
check("ordinary product disclaimer preserves display branding", compliance.includes("a Gabriel Capital Labs platform"));
check("ordinary product disclaimer does not import the server identity", !compliance.includes("companyIdentity.server"));

const envExample = source(".env.example");
check("postal identity is not duplicated into the environment template", !/^COMPANY_POSTAL_ADDRESS=/m.test(envExample));
check("no public company identity/address env is documented", !/NEXT_PUBLIC_COMPANY_(?:LEGAL_NAME|DISPLAY_NAME|POSTAL_ADDRESS)/.test(envExample));
...
const streetSources = productSources.filter((path) => readFileSync(path, "utf8").includes("30 Montgomery St."));
check(
