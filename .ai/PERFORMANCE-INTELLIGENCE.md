...
- **Improvement-measurement integrity (this service MEASURES improvement; the economy REWARDS it — [`CREDITVECTOR-ECONOMY.md §3`](CREDITVECTOR-ECONOMY.md)).** Any "improvement" signal this service emits for reputation to reward must be **un-gameable**: measured against the operator's **best sustained prior baseline** (never a most-recent trough, so deliberate-degrade-then-recover earns nothing), over a **rolling sustained window** (absorbing spikes, regression-to-the-mean, seasonality), gated by **minimum data sufficiency + activity thresholds** (no small-denominator "+100%"), with **org-growth/team-size/client-volume normalization** and a **voided comparison when the SOP/KPI definition changed**. Emit confidence + provenance; flag anomalies for hold. This is the measurement half; the reward-fairness half (cohorts, floors, manual review) is the economy's.
