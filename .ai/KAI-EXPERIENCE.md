...
| **Weekly** | email, opt-in | personal progress (letters, responses, deadlines) + Brief highlights | extends the BUILT Brief digest sender (`lib/briefDigest.ts`) — canonical LLC postal footer + unsubscribe; CCO pass and received-test verification |
...
| **E3** | Digests (weekly personal → monthly → agency) | E1 + deployed/received-test verification of the Founder-resolved legal footer |
