...
export function entitlementSnapshot(opts: { premium: boolean }): EntitlementSnapshot {
  const caps = new Set<CapabilityKey>([DRAFT, OBSOLESCENCE, INSIGHT, COMPOSE]); // deterministic → all users
  const perms = new Set<Permission>(["letters:generate", "obsolescence:check", "tradeline:read", "campaign:compose"]);
  if (opts.premium) { caps.add(ANALYZE); perms.add("responses:analyze"); }     // AI tools → paid
  return {
    grantedCapabilities: caps,
    flags: new Map<CapabilityKey, boolean>([[DRAFT, true], [ANALYZE, true], [OBSOLESCENCE, true], [INSIGHT, true], [COMPOSE, true]]),
