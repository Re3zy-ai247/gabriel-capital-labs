...
Status: **Principles RATIFIED (founder-approved 2026-07-20 via [`ADR-0037`](ADR/ADR-0037-operator-growth-constitution.md)/[`ADR-0038`](ADR/ADR-0038-professional-growth-economy.md)) · §5 ledger architecture IMPLEMENTED (Sprint 10, 2026-07-21, branch `feat/operator-reputation` — `lib/reputation/**` + migration `20260721160000_operator_reputation`, dormant behind `OPERATOR_REPUTATION_ENABLED`, un-merged/un-applied) · all policy VALUES still PROPOSED (owner-gated; zero milestone definitions, zero new weights).** Implementation record: [`RELEASE-sprint10-operator-reputation.md`](RELEASE-sprint10-operator-reputation.md). Ratification of the invariants does NOT imply legal approval or production readiness.
...
- **Issuance integrity:** server-authoritative · append-only · **`UNIQUE(subjectId, operatorId, awardKind)`** — the §5 canonical key (an earlier draft here said `sourceEventId`, the very key §5.1 rejects as BLOCKER-grade because per-emission ids double-count on outcome flips; corrected 2026-07-21 to match §5 + the shipped `XpAward` model). `subjectId` is **version-free** (a version-embedded subject key double-mints on a policy re-weight); `policyVersion` is a recorded column, never part of the key · versioned policies · no hidden mutable balance · separation of duties (the service that *detects* eligibility is not the one that *approves* a manual adjustment) · admin adjustments are themselves audited append-only records · policy rollout behind fail-closed flags.
